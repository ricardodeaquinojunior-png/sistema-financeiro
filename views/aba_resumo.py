...
class AbaResumo:
    def __init__(self, parent):
        ...
        lbl_titulo = tk.Label(self.main_frame, text="Visão Geral do Sistema Financeiro", font=("Arial", 14, "bold"), bg="#F0F0F0", fg="#1A365D")
        ...
        # Card Saldo Efetivo
        self.card_efetivo = self.criar_card(frame_cards, "Saldo Efetivo (Até Hoje)", "R$ 0,00", "#2F855A")
        self.card_efetivo.pack(side="left", fill="x", expand=True, padx=(0, 8))

        # Card Saldo Previsto
        self.card_previsto = self.criar_card(frame_cards, "Saldo Previsto Geral", "R$ 0,00", "#2B6CB0")
        self.card_previsto.pack(side="left", fill="x", expand=True, padx=8)

        # Card Receitas do Mês
        self.card_receita = self.criar_card(frame_cards, "Receitas (Mês Atual)", "R$ 0,00", "#319795")
        self.card_receita.pack(side="left", fill="x", expand=True, padx=8)

        # Card Despesas do Mês
        self.card_despesa = self.criar_card(frame_cards, "Despesas (Mês Atual)", "R$ 0,00", "#C53030")
        self.card_despesa.pack(side="left", fill="x", expand=True, padx=(8, 0))
        ...
        frame_esq = tk.LabelFrame(frame_inferior, text=" Despesas por Categoria / Subcategoria (Duplo clique expande/filtra, Duplo fora limpa) ", font=("Arial", 10, "bold"), bg="#F0F0F0", padx=10, pady=10)
        frame_esq.pack(side="left", fill="both", expand=True, padx=(0, 8))

        colunas_cat = ("Categoria", "Total Cat.", "Subcategoria", "Total Sub.")
        self.tabela_cat = ttk.Treeview(frame_esq, columns=colunas_cat, show="headings", height=10)
        
        for col in colunas_cat:
            self.tabela_cat.heading(col, text=col, command=lambda c=col: self.ordenar_tabela(self.tabela_cat, c, False))
        ...
        frame_total_cat = tk.Frame(frame_esq, bg="white", bd=1, relief="solid", padx=8, pady=5)
        frame_total_cat.pack(fill="x", pady=(5, 0))
        tk.Label(frame_total_cat, text="Total Despesas:", font=("Arial", 8, "bold"), fg="#718096", bg="white").pack(side="left")
        self.lbl_total_cat = tk.Label(frame_total_cat, text="R$ 0,00", font=("Arial", 10, "bold"), fg="#C53030", bg="white")
        self.lbl_total_cat.pack(side="right")

        # Coluna Direita: Movimentações por Período de Cartão ou Mês
        frame_dir = tk.LabelFrame(frame_inferior, text=" Movimentações por Período do Cartão / Mês ", font=("Arial", 10, "bold"), bg="#F0F0F0", padx=10, pady=10)
        ...
        tk.Label(frame_filtro_cartao, text="Cartão:", font=("Arial", 8), bg="#F0F0F0").pack(side="left", padx=(0, 2))
        self.cb_filtro_cartao = ttk.Combobox(frame_filtro_cartao, width=15, state="readonly")
        self.cb_filtro_cartao.pack(side="left", padx=(0, 5))
        self.cb_filtro_cartao.bind("<<ComboboxSelected>>", self.ao_mudar_cartao_resumo)

        tk.Label(frame_filtro_cartao, text="Período/Ciclo:", font=("Arial", 8), bg="#F0F0F0").pack(side="left", padx=(0, 2))
        self.cb_filtro_ciclo = ttk.Combobox(frame_filtro_cartao, width=22, state="readonly")
        self.cb_filtro_ciclo.pack(side="left", padx=(0, 5))
        self.cb_filtro_ciclo.bind("<<ComboboxSelected>>", lambda e: self.carregar_movimentacoes_mes(self.categoria_selecionada_filtro))

        btn_limpar_cartao = tk.Button(frame_filtro_cartao, text="Limpar", font=("Arial", 8), command=self.limpar_filtro_cartao)
        btn_limpar_cartao.pack(side="left")

        colunas_ult = ("Data", "Descrição", "Subcategoria", "Valor", "Status")
        self.tabela_ult = ttk.Treeview(frame_dir, columns=colunas_ult, show="headings", height=8)
        
        for col in colunas_ult:
            self.tabela_ult.heading(col, text=col, command=lambda c=col: self.ordenar_tabela(self.tabela_ult, c, False))
        ...
        self.tabela_ult.bind("<Double-1>", lambda e: self.resetar_filtros_tabela())

        frame_total_mov = tk.Frame(frame_dir, bg="white", bd=1, relief="solid", padx=8, pady=5)
        frame_total_mov.pack(fill="x", pady=(5, 0))
        tk.Label(frame_total_mov, text="Soma do Período/Filtro:", font=("Arial", 8, "bold"), fg="#718096", bg="white").pack(side="left")
        self.lbl_total_mov = tk.Label(frame_total_mov, text="R$ 0,00", font=("Arial", 10, "bold"), fg="#2B6CB0", bg="white")
    ...
    def criar_card(self, parent, titulo, valor_inicial, cor_texto):
        card_frame = tk.Frame(parent, bg="white", bd=1, relief="solid", padx=12, pady=10)
        
        lbl_tit = tk.Label(card_frame, text=titulo, font=("Arial", 8, "bold"), fg="#718096", bg="white")
        lbl_tit.pack(anchor="w")

        lbl_val = tk.Label(card_frame, text=valor_inicial, font=("Arial", 13, "bold"), fg=cor_texto, bg="white")
    ...
    def ordenar_tabela(self, treeview, coluna, reverse):
        ...
        for col in treeview["columns"]:
            if col == coluna:
                treeview.heading(col, text=f"{col}{seta}", command=lambda c=col: self.ordenar_tabela(treeview, c, not reverse))
            else:
                treeview.heading(col, text=col, command=lambda c=col: self.ordenar_tabela(treeview, c, False))

    def carregar_combos_cartoes(self):
        try:
            conn = conectar_banco()
            cursor = conn.cursor()
            cursor.execute("SELECT id, nome, dia_fechamento, dia_vencimento FROM cartoes ORDER BY nome;")
            ...
            for cid, cnome, c_fech, c_venc in cartoes:
                ...
                self.map_cartao_detalhes[cid] = {"fechamento": c_fech or 24, "vencimento": c_venc or 1}
    ...
    def carregar_dados(self):
        try:
            ...
            # 1. Saldos Gerais (Efetivo e Previsto)
            ...
            des_ef = sum([r[1] for r in efetivo if r[0] == 'Despesa'])
            saldo_ef = rec_ef - des_ef

            rec_pr = sum([r[1] for r in previsto if r[0] == 'Receita'])
            des_pr = sum([r[1] for r in previsto if r[0] == 'Despesa'])
            saldo_pr = rec_pr - des_pr

            self.card_efetivo.lbl_valor.config(text=self.formatar_moeda(saldo_ef))
            self.card_previsto.lbl_valor.config(text=self.formatar_moeda(saldo_pr))
            ...
            # 3. Carregar Tabela Esquerda (Categorias e Subcategorias)
            self.atualizar_tabela_categorias()

            # 4. Popula os ciclos do cartão padrão ou mês atual na tabela direita
    ...
    def popular_combobox_ciclos(self):
        ...
        if cartao_id:
            ...
            try:
                conn = conectar_banco()
                cursor = conn.cursor()
                cursor.execute("SELECT MIN(data_lancamento), MAX(data_lancamento) FROM lancamentos WHERE id_cartao = %s;", (cartao_id,))
                res = cursor.fetchone()
                cursor.close()
                conn.close()

                # Tratamento seguro se a data vier como datetime ou date
                def para_date(val):
                    if not val:
                        return None
                    if hasattr(val, 'date'):
                ...
                min_data = para_date(res[0]) if res and res[0] else date.today() - relativedelta(months=3)
                max_data = para_date(res[1]) if res and res[1] else date.today() + relativedelta(months=6)

                curr_date = date(min_data.year, min_data.month, 1)
                end_date = date(max_data.year, max_data.month, 1) + relativedelta(months=2)
                ...
                while curr_date <= end_date:
                    ...
                    rotulo = f"Fatura {data_fim.strftime('%m/%Y')} ({data_ini.strftime('%d/%m/%Y')} a {data_fim.strftime('%d/%m/%Y')})"
        ...
        try:
            ...
            if self.categoria_selecionada_filtro:
                ...
                for c_desc, s_desc, s_val in subs:
                    ...
                    self.tabela_cat.insert("", "end", values=(c_desc, self.formatar_moeda(total_cat_filtrada), s_nome, self.formatar_moeda(v_sub)))
                ...
                for cat_desc, cat_val in cats:
                    ...
                    self.tabela_cat.insert("", "end", values=(cat_desc, self.formatar_moeda(v_cat), "", ""))
        ...
        try:
            ...
            if cartao_id:
                ...
                if ciclo_idx >= 0 and ciclo_idx < len(self.ciclos_disponiveis_cache):
                    ...
                    query += " AND l.data_lancamento >= %s AND l.data_lancamento <= %s"
                    params.extend([d_ini, d_fim])
            else:
                if ciclo_idx >= 0 and ciclo_idx < len(self.ciclos_disponiveis_cache):
                    _, mes_str = self.ciclos_disponiveis_cache[ciclo_idx]
                    query += " AND TO_CHAR(l.data_lancamento, 'YYYY-MM') = %s AND l.id_cartao IS NULL"
            ...
            for data, desc, sub_desc, val, rec, tipo in movimentos:
                ...
                self.tabela_ult.insert("", "end", values=(data_str, desc or "", sub_str, val_formatado, status_str))
