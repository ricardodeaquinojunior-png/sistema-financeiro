...
class AbaLancamentos:
  ...
  def __init__(self, parent):
    ...
    self.cb_tipo = ttk.Combobox(
        frame_form,
        values=["Receita", "Despesa", "Transferência"],
        width=30,
        state="readonly",
    )
    ...
    # Valor (Com digitação fluida e formatação em tempo real)
    self.lbl_valor = tk.Label(
        frame_form, text="Valor da Receita:", bg="#F0F0F0", font=("Arial", 9)
    )
    self.lbl_valor.pack(anchor="w", pady=(2, 0))
    self.var_valor = tk.StringVar(value="0,00")
    self.txt_valor = tk.Entry(
        frame_form, textvariable=self.var_valor, width=32, font=("Arial", 10)
    )
    ...
    self.chk_status = tk.Checkbutton(
        frame_form,
        text="Recebido",
    ...
    self.var_data.trace_add(
        "write", lambda *args: aplicar_mascara_data(None, self.var_data)
    )
    self.txt_data = tk.Entry(
        frame_form, textvariable=self.var_data, width=32, font=("Arial", 10)
    )
    self.txt_data.pack(anchor="w", pady=(0, 6))

    # Descrição
    tk.Label(
        frame_form, text="Descrição:", bg="#F0F0F0", font=("Arial", 9)
    ).pack(anchor="w", pady=(2, 0))
    ...
    self.lbl_cat = tk.Label(
        frame_form, text="Categoria:", bg="#F0F0F0", font=("Arial", 9)
    )
    ...
    self.lbl_sub = tk.Label(
        frame_form, text="Subcategoria:", bg="#F0F0F0", font=("Arial", 9)
    )
    ...
    self.lbl_conta = tk.Label(
        frame_form, text="Conta Bancária:", bg="#F0F0F0", font=("Arial", 9)
    )
    ...
    self.lbl_conta_destino = tk.Label(
        frame_form, text="Conta de Destino:", bg="#F0F0F0", font=("Arial", 9)
    )
    self.cb_conta_destino = ttk.Combobox(frame_form, width=30, state="readonly")

    # Repetir
    self.lbl_repetir = tk.Label(
        frame_form, text="Repetir (Qtd de vezes):", bg="#F0F0F0", font=("Arial", 9)
    )
    ...
    self.btn_salvar = tk.Button(
        ...
        fg="white",
        font=("Arial", 9, "bold"),
        command=self.salvar_ou_alterar,
    )
    ...
    self.btn_limpar = tk.Button(
        frame_botoes, text="Limpar", font=("Arial", 9), command=self.limpar_campos
    )
    ...
    btn_config_colunas = tk.Button(
        header_tabela,
        text="⚙ Colunas",
        font=("Arial", 8, "bold"),
        command=self.abrir_config_colunas,
    )
    ...
    self.cb_filtro_conta = ttk.Combobox(
        sub_header_conta, width=35, state="readonly"
    )
    self.cb_filtro_conta.pack(side="left")
    self.cb_filtro_conta.bind(
        "<<ComboboxSelected>>", lambda e: self.carregar_dados()
    )
    ...
    self.tabela = ttk.Treeview(
        frame_tabela_container, columns=self.colunas_nomes, show="headings", height=15
    )

    self.colunas_visiveis = {
        col: tk.BooleanVar(value=True) for col in self.colunas_nomes
    }

    # Tags de Cores com fonte padronizada (Arial, 9)
    self.tabela.tag_configure(
        "tag_vermelho", foreground="#C53030", font=("Arial", 9, "bold")
    )
    self.tabela.tag_configure(
        "tag_azul", foreground="#2B6CB0", font=("Arial", 9, "bold")
    )
    self.tabela.tag_configure(
        "tag_padrao", foreground="#000000", font=("Arial", 9)
    )
    ...
    scrollbar = ttk.Scrollbar(
        frame_tabela_container, orient="vertical", command=self.tabela.yview
    )
    ...
    frame_saldos = tk.Frame(
        frame_direito, bg="white", bd=1, relief="solid", padx=10, pady=8
    )
    frame_saldos.pack(side="bottom", fill="x")

    self.lbl_saldo_efetivo = tk.Label(
        frame_saldos,
        text="R$ 0,00",
        font=("Arial", 11, "bold"),
        fg="#2F855A",
    ...
    self.lbl_titulo_efetivo = tk.Label(
        frame_saldos,
        text="Saldo Atual na Conta (Até Hoje):",
        font=("Arial", 8, "bold"),
        fg="#718096",
        bg="white",
    )
    ...
    self.lbl_saldo_previsto = tk.Label(
        frame_saldos,
        text="R$ 0,00",
        font=("Arial", 11, "bold"),
        fg="#2B6CB0",
    ...
    self.lbl_titulo_previsto = tk.Label(
        frame_saldos,
        text="Saldo Total Previsto (Geral):",
        font=("Arial", 8, "bold"),
        fg="#718096",
        bg="white",
    )
  ...
  def abrir_config_colunas(self):
    ...
    for col in self.colunas_nomes:
      chk = tk.Checkbutton(
          janela_cfg,
          text=col,
          variable=self.colunas_visiveis[col],
          bg="#F0F0F0",
          font=("Arial", 9),
  ...
  def atualizar_colunas_visiveis(self):
    visiveis = [
        col for col in self.colunas_nomes if self.colunas_visiveis[col].get()
    ]
  ...
  def carregar_combos(self):
    try:
      ...
      cursor = conn.cursor()
      cursor.execute(
          "SELECT id_categoria, descricao FROM categorias ORDER BY descricao;"
      )
      ...
      cursor.execute(
          "SELECT id, instituicao, principal FROM contas ORDER BY instituicao;"
      )
      ...
      if (
          conta_principal_nome
          and not self.cb_conta.get()
          and not self.lancamento_selecionado_id
      ):
        self.cb_conta.set(conta_principal_nome)
      if (
          lista_contas
          and not self.cb_conta_destino.get()
          and len(lista_contas) > 1
      ):
  ...
  def ao_mudar_categoria(self, event=None):
    ...
    if cat_id:
      try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT id_subcategoria, descricao FROM subcategorias WHERE"
            " id_categoria = %s ORDER BY descricao;",
            (cat_id,),
        )
  ...
  def carregar_dados(self):
    ...
    try:
      ...
      id_conta_filtro = (
          self.map_conta.get(conta_filtro_nome)
          if conta_filtro_nome and conta_filtro_nome != "Todas as Contas"
          else None
      )
      ...
      query_normais += (
          f" ORDER BY {self.coluna_ordenacao} {direcao_sql}, l.id DESC"
      )

      cursor.execute(
          query_normais, tuple(params_normais) if params_normais else None
      )
      ...
      if not id_conta_filtro:
        ...
        for (
          ...
          if dia_compra <= fechamento:
            base_venc = datetime(d_data.year, d_data.month, 1) + relativedelta(
                months=1
            )
          else:
            base_venc = datetime(d_data.year, d_data.month, 1) + relativedelta(
          ...
          data_vencimento = datetime(
              base_venc.year, base_venc.month, vencimento_dia
          )
      ...
      dados_exibicao = sorted(
          dados,
          key=lambda x: self.normalizar_data(x[4]),
          reverse=(not self.ordem_ascendente),
      )

      conn_hist = conectar_banco()
      cur_hist = conn_hist.cursor()
      q_hist = (
          "SELECT tipo, valor, data_lancamento FROM lancamentos WHERE id_cartao"
          " IS NULL"
      )
      p_hist = []
      if id_conta_filtro:
      ...
      menor_data_tela = (
          self.normalizar_data(dados_exibicao[0][4])
          if self.ordem_ascendente
          else self.normalizar_data(dados_exibicao[-1][4])
      )
      ...
      for item in dados_exibicao:
        ...
        if lrec:
          status_str = (
              ...
              if ltipo == "Despesa"
                  ...
                  "Realizada" if ltipo == "Transferência" else "Recebido"
              )
          )
        else:
          status_str = "Pendente"
        ...
        self.tabela.insert(
            ...
            tags=(tag_linha, str(lid)),
        )

      # --- LÓGICA DE FOCO AUTOMÁTICO (Hoje ou a data mais próxima) ---
      itens_tabela = self.tabela.get_children()
      if itens_tabela:
        hoje = datetime.now().date()
        melhor_item = None
        menor_diff = float("inf")

        for item_id in itens_tabela:
          valores = self.tabela.item(item_id, "values")
          if valores and valores[0] != "---":
            try:
              dt_linha = datetime.strptime(valores[0], "%d/%m/%Y").date()
              diff = abs((dt_linha - hoje).days)
              if diff < menor_diff:
                menor_diff = diff
                melhor_item = item_id
            except ValueError:
        ...
        if melhor_item:
          self.tabela.selection_set(melhor_item)
          self.tabela.focus(melhor_item)
          self.tabela.see(melhor_item)
  ...
  def calcular_saldos(self, id_conta=None):
    try:
      ...
      if id_conta:
        ...
        cursor.execute(
            "SELECT tipo, SUM(valor) FROM lancamentos WHERE id_conta = %s GROUP"
            " BY tipo;",
            (id_conta,),
        )
        previsto = cursor.fetchall()
        nome_conta_sel = self.cb_filtro_conta.get()
        self.lbl_titulo_efetivo.config(
            text=f"Saldo Atual na Conta '{nome_conta_sel}' (Até Hoje):"
        )
        self.lbl_titulo_previsto.config(
            text=f"Saldo Total Previsto para '{nome_conta_sel}':"
        )
      else:
        cursor.execute(
            "SELECT tipo, SUM(valor) FROM lancamentos WHERE recebido = TRUE AND"
            " data_lancamento <= CURRENT_DATE GROUP BY tipo;"
        )
        efetivo = cursor.fetchall()
        cursor.execute(
            "SELECT tipo, SUM(valor) FROM lancamentos GROUP BY tipo;"
        )
        previsto = cursor.fetchall()
      ...
      des_ef = sum(
          [r[1] for r in efetivo if r[0] in ["Despesa", "Transferência"]]
      )
      saldo_ef = rec_ef - des_ef

      rec_pr = sum([r[1] for r in previsto if r[0] == "Receita"])
      des_pr = sum(
          [r[1] for r in previsto if r[0] in ["Despesa", "Transferência"]]
      )
      saldo_pr = rec_pr - des_pr

      self.lbl_saldo_efetivo.config(
          text=f"R$ {saldo_ef:,.2f}"
          .replace(",", "X")
          .replace(".", ",")
          .replace("X", ".")
      )
      self.lbl_saldo_previsto.config(
          text=f"R$ {saldo_pr:,.2f}"
          .replace(",", "X")
          .replace(".", ",")
          .replace("X", ".")
      )
  ...
  def selecionar_registro(self, event):
    ...
    if str(lid).startswith("fat_"):
      messagebox.showinfo(
          "Informação",
          "Esta é uma fatura consolidada de cartão. Para alterá-la, gerencie"
          " os lançamentos individuais na aba 'Despesas de Cartão'.",
      )
      return
    ...
    try:
      ...
      if reg:
        ...
        self.var_data.set(
            data_lanc.strftime("%d/%m/%Y") if data_lanc else ""
        )
        ...
        cor_botao = (
            "#C53030"
            if tipo == "Despesa"
            else ("#D69E2E" if tipo == "Transferência" else "#D69E2E")
        )
  ...
  def salvar_ou_alterar(self):
    ...
    if (
        tipo == "Transferência"
        and self.cb_conta.get() == self.cb_conta_destino.get()
    ):
    ...
    try:
      ...
      if tipo == "Transferência":
        ...
        id_sub = (
            self.map_sub.get(self.cb_sub.get())
            if self.cb_sub.get()
            else None
        )
        ...
        if self.lancamento_selecionado_id:
          ...
          for i in range(repeticoes):
            ...
            desc_final = (
                f"{desc} ({i+1}/{repeticoes})" if repeticoes > 1 else desc
            )
  ...
  def excluir_lancamento(self):
    ...
    if messagebox.askyesno(
        "Confirmar", "Tem certeza que deseja excluir este lançamento?"
    ):
      try:
        conn = conectar_banco()
        cursor = conn.cursor()
